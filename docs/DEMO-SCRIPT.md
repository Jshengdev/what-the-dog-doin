...
| 3.1c | the LiDAR band drawn around the orange dog | blue dots on the walls and furniture around the dog, moving with it | "lidar on" (`POST /dog/lidar {on}`) | **proven live** (about 8 frames a second, odometry frame, 1000+ points) | "its own lidar, on our drawing" |
