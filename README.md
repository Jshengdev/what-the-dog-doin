...
It orchestrates three external systems: the Unitree Go2 (the body: it walks the route, looks, photographs, scans), the housemates' iMessage group (where it is asked, where it reports, where it is answered and corrected), and the room's lights, Philips Hue and a Tuya strip (the beacon that follows it and the alarm). One Python agent, `wtdd/`, runs the routine as a fixed sequence of tools across the three, with a vision model writing the words and a ledger row confirming every step from the device it touched.
...
The premise: an agent that can see and act in the real world can do real work. This one has a body, and its job is a house with roommates.
...
| Demo clarity | The video: text the group, the dog goes, the lights follow, the socks get reported, the stranger gets asked about, "dog done". | [the video](https://youtu.be/8IqdF3_FY70); `docs/DEMO-SCRIPT.md` is the shot list with the receipt to point at per shot |
...
| 6 | at a stop recorded as the intruder check, a person in the frame: "who dis?!" with the photo, and the round holds up to 45 s for the group's answer. "idk" and its kin: "STRANGER DANGER!!!" three times, the living room strobes red and blue for five seconds, every light is put back as it was; anything else: "ok, standing down"; silence: it moves on | iMessage, Hue, Tuya | post `alarm:<guid>:<stop>`, `wtdd/chat/listen.py` verdict(), `light_alarm` | `chat.post`, `intruder.verdict`, `lights.alarm`, `lights.signal` x4, `lights.tuya_set` |
...
<p align="center"><img src="docs/media/chat-dogs-eye-view.png" alt="two photos the group received: a level look across the desk, and a nod-up look over the coffee table" width="420"><br><sub>docs/media/chat-dogs-eye-view.png: what the group gets from "send room shot to castle" on the remote: the dog's eye view, a level look and a nod up, from the remote</sub></p>

**Where it thinks it is.** The Go2's Wi-Fi driver exposes no navigation, so the map knowledge is ours: one calibration ties the dog's odometry (20 Hz, drifts) to a map point and heading (`wtdd/dog/nav.py`, 108.5 px per metre). On the remote the dog is an orange dot with a cone for where it points; drag the dot to where it really is, drag the cone tip to where it looks, and that is a `dog.calibrate` row. **Teach the route by driving:** "record route" traces the believed position while the dog is driven with the controller; any look pressed on the way ("nod + photo", "nod + say to castle", a level or sit photo) is performed right there and recorded on the route at that spot as the action to replay, "mark stop here" drops a default one, "stop & save route" writes the thinned trace as the map's path with its stops and actions. **Planned routes:** `plan_path` runs A* over the drawn rooms between any two map points and can write the result as the route (`save=true`). The floor plan draws rooms as rectangles without walls or doors between them yet, so a planned route may cross a shared wall; that is why the demo route is recorded by driving. **Replay:** "walk the path" drives it waypoint by waypoint (proportional steering, 0.3 m/s, avoidance on and read back first, a waypoint not reached in 30 s fails loud), the lights following its believed position, and at each stop it does what was recorded there: the look, and the post when the recording said so, each keyed on the stop so a replay never posts twice for it. A path with a point outside every room or a jump over 300 px is refused by name before anything moves. Anyone in the frame counts as a stranger: recognizing housemates is not built.

## The three integrations

The agent's job is to move one routine across three systems that know nothing of each other and to prove each step from the system's own answer. Each integration below says what it is, how the agent talks to it, what it does in the round, and how every action is confirmed.

### 1. Unitree Go2, the body

What it does in the round: drives the recorded route on its odometry, holds at each stop, nods or sits to take the pictures, streams the live camera the detector watches and the LiDAR voxel map the plan draws, and is halted by a person's "stop" or its own waypoint timeout. How it works: One WebRTC session to the Go2 over the community driver `unitree_webrtc_connect` 2.2.0, held by the API process for its lifetime; the dog accepts one peer and keeps the slot for about ten seconds after a close, so every other process (the chat listener, the CLI) reaches the dog through `POST /tools/<name>` and never opens a second peer. Connects are serialized by a lock, a state stream older than 5 s marks the session dead and the next call reconnects once (no loop), and an unreachable dog is probed at most every 15 s so a dead dog doesn't flood the ledger. Commands are an allowlist (Sit, RiseSit, StandUp, StandDown, BalanceStand, StopMove, Move, Hello, Stretch, Pose, Euler…; no flips, no jumps), each sent on the sport topic and confirmed by a fresh 20 Hz state sample before its row is written. This firmware runs motion mode `mcf` and refuses `normal` (code 7004); the same command ids work. Hold-to-drive is a velocity the remote refreshes every 200 ms and a loop that republishes at 10 Hz and stops 0.6 s after the last refresh, so a dropped key never runs it into a wall. The nod that takes the pictures is a measured recipe: StopMove, BalanceStand, Pose on, Euler +0.3 (nose down, +15° at 0.7 s, frame), 1.6 s, Euler −0.3 (nose up, −15° from 0.36 to 0.79 s, frame at 0.6 s), Euler 0, Pose off; the pose is a nod, not a hold, and only fires as this pair. The IMU pitch at capture is on the row; below 8° the nod is retried once after a StandUp, and a pose refused with code 401001 (after the physical controller drove it or it was carried) is cured by Sit then RiseSit, which StandUp does not do. Frames come from the live video track through PyAV (cv2 must never load in that process: both wheels bundle libavdevice), hashed on the row. How it is confirmed: every command waits for a fresh state sample before its row is written, every frame carries its sha256 and the IMU pitch at capture, every route replay is a `dog.follow` row with the waypoints reached and the believed end.

### 2. iMessage, the housemates' group chat

What it does in the round: the trigger ("what the dog doin" from any member), every report (the picture, "dog doin", each stop's boxed photo and sentence, "who dis?!", "STRANGER DANGER!!!", "dog done" with any failure named), the verdicts and corrections it takes back, and the "yo dog" chat turns. How it works: The listener polls `~/Library/Messages/chat.db` by ROWID every 2 s from `MAX(ROWID)` at boot (no replay), stores every message it sees in `memory.db`, and answers only three things: a wake phrase, a chat prefix, or a reply to an open question. Wake is fuzzy on purpose (housemates misspell): a phrase inside the text, or a sliding window at ≥ 0.80 similarity, or the tokens "dog" and "doin"; "the dog is cute" and "hotdog time" do not wake (tested). Every post goes through one function: the target gate (the guid must equal `WTDD_CHAT_GUID` and that chat's display name must equal `WTDD_CHAT_NAME`, both checked again inside the sender), the never-twice claim (an INSERT keyed on the causing message before osascript runs; a conflict means no send), the AppleScript send, and confirmation by reading the dog's own from-me row back within 10 s; an unconfirmed send raises and is never resent. Photos are staged under `~/Pictures/wtdd` (sandboxed Messages can read there) and their caption is a second confirmed row. The dog never reads its own words (confirmed guids and opening words are refused). A "yo dog" turn gathers the sender's next 6 s of messages, hands the group's context to the model (who said what, the last ledger rows, the corrections) and posts one answer; nothing else is answered. "who dis?!" opens a question the next answer settles; inside a round the listener reads the chat at the stop for up to 45 s. How it is confirmed: every send is a `chat.gate` row, a `chat.claim` row and a `chat.post` row whose `state_after` is the dog's own message read back from `chat.db` by guid; a send that cannot be read back is an error, never a retry.

### 3. The lights, Philips Hue and the Tuya strip

What they do in the round: the beacon and the alarm. The living room's four Hue lights and the strip follow the body by proximity, brighter as it nears, dark behind it, so where the light is on is where the dog is looking; on "STRANGER DANGER!!!" they strobe red and blue and are put back. How it works: A proximity field: each lamp's level is `(1 − d/R)^k × gain`, clipped at 100, with R = 350 px (3.2 m), k = 1.6, gain 2.1 (so 1.6 m from a lamp is 70% and within 1.2 m is full); the strip is the mean of that over 12 samples along its line, so a graze at its end is dim and a pass over its middle bright; lights in rooms the body is not in are scaled by 0.3. The entity is either the simulated walker on the drawn path or the real dog's believed pose. Writes go through the registry, one thread per light and at most one write in flight per light, so each light steps exactly as fast as its own measured latency allows: Hue through the cloud Remote API about 0.8 s per set including the read-back GET, the Tuya strip over local protocol 3.5 about 0.4 s (it does not ack writes, so it is written nowait and read back). A write is only sent when a level moved by 4 points or crossed zero. The room starts dark and ends dark, and both are waited for. A walk refuses to start while another is live (`field.json` written at 10 Hz is the lock), and the remote greys out every light button meanwhile. The alarm signals red/blue on all four Hue lights natively (the bridge's `alternating` signal), pushes the strip to 100, waits its seconds, and puts every light back exactly as it was read before, read back again. How it is confirmed: every write is one `lights.set` or `lights.tuya_set` row with the device state read back after it; the `walk` trials count writes and failures per light.

### The model

OpenRouter, `x-ai/grok-4.20`: reads the two nod frames with the detector's labels and returns JSON (the sentence, whether a person is there, what is out of place, which frame to send, its check on the detector), and answers "yo dog" turns as function calls over the same tools. Every call is an `llm.generate` row with model, message and image counts, tokens, latency and the raw reply. No retries, no fallback model; a malformed reply is an error on its row.
...
3. At waypoint 14 the follower stops 56 px short and times out after 30 s (the dog's obstacle-avoidance service had stopped answering after a battery swap, so the run was driving without it, by explicit choice, and that is on the `dog.follow` row). The round reports it and continues: it looks where it stands, finds someone at the table ("it says chair x2 but that is really a backpack and a person"), posts the photo, asks **"who dis?!"** and holds.
...
This is hardware in a real house. It was tested by running the job, reading what it wrote down, and trying to break it.
...
**Two eyes, and the second one checks the first.** The detector (YOLO11n, COCO classes) runs first and boxes the floor frame. Its labels go into the vision model's call with both frames. The model returns what's out of place, whether a person is there, which picture to send, and `detector_check`: on the real frame below the detector said "bird"; the model answered "it says bird but that is really a sock" and that went into the message. Each look writes a `vision.check` row with both opinions side by side.
...
| the detector mislabels something (a "bird" that was socks) | the detector runs first and its labels go into the vision model's call; the model answers `detector_check` ("it says bird but that is really a sock") and folds it into the sentence | a `vision.check` row per look: the detector's labels, the model's list, agree or the correction clause; the caption carries both | seen on the real frame 2026-09-13 |
...
Re-running never re-posts: every post is claimed on the guid of the message that caused it (plus the stop index), a CLI post on a key given on the command line. Re-running the round rewrites the same lights to the same levels and reads them back; the dog gets the same allowlisted commands. Nothing is deleted from the group, ever.
...
One model through OpenRouter (`wtdd/llm.py`, no retries, no fallback model): `x-ai/grok-4.20`, the fastest of six measured on a live dog frame (about 1 s) and 3.3 s on a tool-calling ask; it reads the frames and answers "yo dog" turns as function calls over the same tool registry (`wtdd/agent.py`). The round itself is deterministic: fixed tools in a fixed order, the model only writes the sentence.

## How it's built, subsystem by subsystem

The three integrations are described above; the rest of the system follows. Every fact was measured on the real devices on 2026-09-13; the module docstrings carry the same facts next to the code.

**Where it is (`wtdd/dog/nav.py`, `session.py`).** The driver exposes no navigation, so the map knowledge is ours. One calibration ties the odometry pose (metres, IMU yaw, frame fixed at power-on, drifts) to a map point and heading at 108.5 px per metre (the living room is 445 px for 4.1 m); heading is radians clockwise on screen because y is down, and a positive ROS yaw is a left turn. A person corrects drift by dragging the dot (each drag a `dog.calibrate` row; 16 today, median 0.22 m). A route is taught by driving: the believed pose is sampled at 5 Hz, thinned to a waypoint every 45 px (about 0.4 m), and every look pressed on the way is stored at that spot with its kind and whether to report. The follower is a proportional controller at 10 Hz: turn toward the next waypoint at up to 0.5 rad/s, move at 0.3 m/s only while the heading error is under 0.6 rad, a waypoint counts within 30 px, 30 s without reaching one fails the run loud; it pauses at stops until the action there is done. Velocities go through the dog's own obstacle-avoidance service when it answers (`dog.avoid` rows with read-back); that service keeps the last velocity until told zero, which is why every stop sends a zero through it before StopMove (found live: the dog kept walking after StopMove alone). Following without it is an explicit choice recorded on the `dog.follow` row. The LiDAR voxel map (128 × 128 × 38 cells at 5 cm, LZ4 frames decoded by the driver's native path, frame `odom`) is drawn as the band between 10 cm and 1 m over the plan; the first live frame's window centre matched the odometry position within 4 cm.

**The eye (`wtdd/watch.py`, `wtdd/tools/dog_say.py`, `wtdd/llm.py`).** Two detectors. YOLO11n (COCO, open source) runs in its own process pulling the live frame from the API, 20 ms a frame on this Mac, boxes drawn to a JPEG the remote shows, a `watch.detect` row only when the set of classes in view changes and holds for three frames (no flicker rows); it also boxes the exact frame a look posts, in a one-shot subprocess. The vision model (`x-ai/grok-4.20`, the fastest of six measured on a live dog frame, about 1 s) gets both nod frames plus the detector's labels and returns JSON: `say`, `person`, `out_of_place`, `pick` (which frame to send), `why`, and `detector_check` ("agree", or what the mislabeled thing really is). A malformed or empty reply raises; there is no canned sentence. The prompt has no example object: it reports only what's in the pictures, guesses an owner by first name only for something that's really there, shames a cup left out, asks whose the socks are. Housemate corrections are a scoped hint (apply only if the same thing is in view). Every call is one `llm.generate` row with model, message and image counts, tokens, latency and the raw reply; `vision.check` rows put the detector and the model side by side.

**The map and the remote (`wtdd/api.py`, `ui/`).** A stdlib HTTP server on 127.0.0.1 over the tool registry, plus one static page (React 18 from esm.sh with htm, no build step). The page never computes a light: it draws `GET /field` (the running walk's position, room and levels, 10 Hz), `GET /dog/state` (odometry, IMU, the believed pose, the follower's status), `GET /watch`, `GET /dog/lidar`, `GET /ledger`, `GET /chat` (the listener's heartbeat). Saves carry the map file's version and a stale page is refused (a recording was lost to exactly that before the guard). The demo view has the buttons the round needs; `#admin` has everything.

**The ledger and the evals (`wtdd/ledger.py`, `wtdd/evals.py`).** `step()` is a context manager that writes exactly one row per call, ok or raised, with `state_before`, `state_after`, the raw response or the error, and latency; an exception is recorded on its row and re-raised, never swallowed. Rows carry a `run_id` per process and `source: live`. The evals run real scenarios and grade each trial from the rows it appended and the devices' read-backs: `walk` (0 write errors, every write ok, every light one of the living room's five), `look` (the nod fired by IMU and the sentence names the planted object), `person` (the JSON says person), `twice` (two wakes make one show, a second claim is refused), `follow` (every waypoint reached, no error). `unsafe` is asserted from the same rows: a post without a claim, a post twice on one trigger, a Hue write outside the living room, a dog command outside the allowlist. `--write` merges each scenario's newest trials into `evals.json` and regenerates the table in this README.

**Bugs found and fixed on the day, from the logs:** the page never defined its API base (the dog panel had rendered nothing from the browser); a photo caption's guid was not on the never-twice table, so the listener could read the dog's own sentence and fuzzy-match "sits" to "sit"; two walks could drive the same lights at once; dry runs counted writes that never happened; the unsafe check skipped the alarm's signal rows; the avoidance service kept the last velocity after StopMove; the pose refused with 401001 needed a sit, not a StandUp; a stale page overwrote a 23-point recording; a killed process left a stale walk file that greyed out the page. Each has a commit and, where possible, a live proof.
