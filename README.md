...
**Bugs found and fixed on the day, from the logs**, because a build that says which is worth more than one that says none: the page never defined its API base (the dog panel had rendered nothing from the browser); a photo caption's guid was not on the never-twice table, so the listener could read the dog's own sentence and fuzzy-match "sits" to "sit"; two walks could drive the same lights at once; dry runs counted writes that never happened; the unsafe check skipped the alarm's signal rows; the avoidance service kept the last velocity after StopMove; the pose refused with 401001 needed a sit, not a StandUp; a stale page overwrote a 23-point recording; a killed process left a stale walk file that greyed out the page. Each has a commit and, where it could be, a live proof.
