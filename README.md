...
| 5 | at every stop drawn on the map: nod, two photographs from the one nod (the floor at 15 degrees nose-down, the room at 15 degrees nose-up, both IMU-checked), one call to the vision model with both as JSON (say, person, out_of_place, pick, why), the picked photo posted with the sentence | Unitree Go2 (WebRTC), OpenRouter, iMessage | `dog_say` (`dog_look` + vision + `chat_post`), post `say:<guid>:<stop>` | `dog.look` (both pitches), `dog.frame` x2 (sha256), `llm.generate`, `chat.post` |
...
| the detector or the model mislabels something (a "bird" that was socks) | a housemate says so in the chat within 30 min of the post | `chat.correction` row naming the post, "noted: …" back, the correction in every later look's prompt; nothing is edited or deleted | seen live 2026-09-13 |
