...
Nothing is copied from the original comic; the flames and the caption are drawn here."""
NAME, DOC = "dog_on_fire", __doc__.strip()
...
def run(caption="this is fine"):
    ...
    rnd = random.Random(7)
    for i in range(46):  # flames: layered tongues along the bottom and sides, drawn, not pasted
        x = rnd.randint(0, w)
    ...
    for i in range(12):
        x = rnd.randint(0, w); y = rnd.randint(int(h * 0.3), h)
