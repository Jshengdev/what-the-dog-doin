"""One model call path for the whole package: OpenRouter, OpenAI-compatible chat completions, Claude models.

Text:   reply = generate("central", [{"role":"user","content":"..."}])
Images: generate("watch", messages_with_image_parts, model=VISION_MODEL)  where an image part is
        {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64,<b64>"}}
JSON:   pass response_format={"type": "json_object"} (or a json_schema) and validate the result yourself.
Every call is one ledger row (tool "llm.generate") with model, tokens, latency. No retries, no fallback model
(CLAUDE.md §2): a refusal or a 429 is recorded on this step and raised.
"""
...
def model(kind: str = "text") -> str:
    return config.get("OPENROUTER_VISION_MODEL" if kind == "vision" else "OPENROUTER_MODEL")
