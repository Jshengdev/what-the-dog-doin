"""python -m wtdd.chat <cmd>. See GOAL.md. Every send: gate, claim, ledger step, osascript, read-back, confirm."""
...
from .housemates import HOUSEMATES, is_trigger
...
def post_step(guid: str, trigger: str, kind: str, text: str | None, file: str | None) -> dict[str, Any]:
    """One gated send inside one chat.post ledger row; state_after is the confirmed from-me row {guid, rowid, ts}."""
...
def cmd_watch(a: argparse.Namespace) -> None:
    guid = _guid(a)
    show_text = guid == config.maybe("WTDD_CHAT_GUID")   # full text only for the test group; other chats: handle + length
    ...
    while True:
        ...
        if msgs:
            ...
            last = msgs[-1]["rowid"]
            memory.set_kv("last_rowid", str(last))
            for m in msgs:
                ...
                trig = " TRIGGER" if (not m["is_from_me"] and m["sender"] in HOUSEMATES and is_trigger(m["text"])) else ""
...
def cmd_send(a: argparse.Namespace) -> None:
    post(_guid(a), _trigger(a), "send", text=a.text)


def cmd_update(a: argparse.Namespace) -> None:
    if "\n" in a.text:
        raise SystemExit("[wtdd:chat] update must be one line")
    post(_guid(a), _trigger(a), "update", text=a.text)


def cmd_photo(a: argparse.Namespace) -> None:
    post(_guid(a), _trigger(a), "photo", text=a.text, file=a.file)
...
def cmd_spam(a: argparse.Namespace) -> None:
    ...
    for i, (trigger, text) in enumerate(plan, 1):
        post(guid, trigger, "spam", text=text)
        if i < len(plan):
            time.sleep(a.every)
...
def cmd_reply(a: argparse.Namespace) -> None:
    ...
    if trig is None:
        ...
        raise SystemExit("[wtdd:chat] no trigger-phrase message from a known housemate stored for this chat (run watch; fill HOUSEMATES)")
    claim(trig["guid"])
    text = generate(memory.context(guid, a.n), trig["text"])
    post_step(guid, trig["guid"], "reply", text, None)
...
def cmd_listen(a: argparse.Namespace) -> None:
    ...
    if not a.dry_run:
        gate(guid)
    Listener(guid, post, listen_s=a.listen_s, dry_run=a.dry_run).run(every=a.every, once=a.once)


def cmd_triggers(a: argparse.Namespace) -> None:
    from .triggers import commands, is_wake, match_command, wake_phrases
...
def main(argv: list[str] | None = None) -> int:
    ...
    l = sub.add_parser("listen", help="wake phrase arms the dog; commands from WTDD_COMMANDS are recognized and run")
    l.add_argument("--guid"); l.add_argument("--every", type=float, default=2.0)
    ...
    w = sub.add_parser("watch", help="poll one chat by ROWID, store into memory.db (read-only)")
    w.add_argument("--guid")
    ...
    for cmd, fn, extra in (("send", cmd_send, ()), ("update", cmd_update, ()), ("photo", cmd_photo, ("file",)),
                           ("spam", cmd_spam, ("n", "every"))):
        s = sub.add_parser(cmd)
        s.add_argument("--guid")
        s.add_argument("--text", required=cmd != "photo", default=None)
    ...
    for cmd, fn in (("reply", cmd_reply), ("context", cmd_context)):
        s = sub.add_parser(cmd)
        s.add_argument("--guid")
